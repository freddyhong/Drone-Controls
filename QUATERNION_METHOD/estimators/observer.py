...
class Observer:
    def __init__(self, ts: float, initial_measurements: MsgSensors=MsgSensors()):
        ...
        self.R_pseudo = np.diag([
                10.0,  # pseudo measurement #1 ##### TODO #####
                10.0,  # pseudo measurement #2 ##### TODO #####
                10.0,  
                10.0, 
                ])
        self.R_gps = np.diag([
    ...
    def update(self, measurement: MsgSensors) -> MsgState:
        ...
        y_pseudo = np.array([[0.], [0.], [0.], [0.]]) # Changed to 4x1
    ...
    def f_smooth(self, x, u):
        ...
        chi_dot = CTRL.gravity/max(Vg, 1e-6)*np.tan(phi)*np.sin(chi - psi)
        psi_dot = (q*np.cos(phi) + r*np.sin(phi))/np.cos(theta)
        Vg_dot = ((Va*np.cos(psi)+wn)*(-Va*psi_dot*np.sin(psi)) + (Va*np.sin(psi)+we)*(Va*psi_dot*np.cos(psi)))/max(Vg, 1e-6)
    ...
    def h_pseudo(self, x: np.ndarray, u: np.ndarray)->np.ndarray:
        ...
        pn, pe, Vg, chi, wn, we, psi = x.flatten()
        # q, r, Va, phi, theta = u.flatten()
        # Vn = Va*np.cos(psi) + wn - Vg*np.cos(chi)
        # Ve = Va*np.sin(psi) + we - Vg*np.sin(chi)   
        # y = np.array([
        #     [pn],
        #     [pe],
        #     [Vn],
